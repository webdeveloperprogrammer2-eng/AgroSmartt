...
export const chatApi = {
  ...
  async getById(chatId, userId) {
    const res = await apiFetch(`${url}/${chatId}${buildQuery({ userId })}`);
  ...
  async getMessages(chatId, { userId, limit, before } = {}) {
    ...
    const res = await apiFetch(`${url}/${chatId}/messages${query}`);
  ...
  async unreadCount(userId) {
    ...
    const res = await apiFetch(`${url}/unread/count${buildQuery({ userId })}`);
